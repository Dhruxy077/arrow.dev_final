    ...
    const components = useMemo(() => {
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (className?.includes('__arrowArtifact__')) {
          ...
          if (className?.includes('__boltSelectedElement__')) {
            ...
            return (
              <div className="bg-arrow-elements-background-depth-3 border border-arrow-elements-borderColor rounded-lg p-3 my-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono bg-arrow-elements-background-depth-2 px-2 py-1 rounded text-arrow-elements-textTer">
                    {elementData?.tagName}
                  </span>
                  {elementData?.className && (
                    <span className="text-xs text-arrow-elements-textSecondary">.{elementData.className}</span>
                  )}
                </div>
                <code className="block text-sm !text-arrow-elements-textSecondary !bg-arrow-elements-background-depth-2 border border-arrow-elements-borderColor p-2 rounded">
          ...
          if (className?.includes('__arrowQuickAction__') || dataProps?.dataArrowQuickAction) {
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            dataProps?.class?.toString().includes('__arrowQuickAction__') ||
            dataProps?.dataArrowQuickAction === 'true'
          ) {
            ...
            return (
              <button
                className="rounded-md justify-center px-3 py-1.5 text-xs bg-arrow-elements-item-backgroundAccent text-arrow-elements-item-contentAccent opacity-90 hover:opacity-100 flex items-center gap-2 cursor-pointer"
                data-type={type}
 ...
 * const input = "```xml\n<div class='__arrowArtifact__'></div>\n```";
 * stripCodeFenceFromArtifact(input);
 * // Returns: "\n<div class='__arrowArtifact__'></div>\n"
 *
 * @remarks
 * - Only removes code fences that directly wrap an artifact (marked with __arrowArtifact__ class)
 * - Handles code fences with optional language specifications (e.g. ```xml, ```typescript)
 * - Preserves original content if no artifact is found
 * - Safely handles edge cases like empty input or artifacts at start/end of content
 */
export const stripCodeFenceFromArtifact = (content: string) => {
  if (!content || !content.includes('__arrowArtifact__')) {
    return content;
  }

  const lines = content.split('\n');
  const artifactLineIndex = lines.findIndex((line) => line.includes('__arrowArtifact__'));

...
export function LockManager() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Controls */}
      <div className="flex items-center gap-1 px-2 py-1 border-b border-arrow-elements-borderColor">
        {/* Search Input */}
        <div className="relative flex-1">
          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-arrow-elements-textTertiary i-ph:magnifying-glass text-xs pointer-events-none" />
          <input
            type="text"
            placeholder="Search..."
            className="w-full text-xs pl-6 pr-2 py-0.5 h-6 bg-arrow-elements-background-depth-2 text-arrow-elements-textPrimary rounded border border-arrow-elements-borderColor focus:outline-none"
            value={searchTerm}
        ...
        {/* Filter Select */}
        <select
          className="text-xs px-1 py-0.5 h-6 bg-arrow-elements-background-depth-2 text-arrow-elements-textPrimary rounded border border-arrow-elements-borderColor focus:outline-none"
          value={filter}
      ...
      <div className="flex items-center justify-between px-2 py-1 text-xs text-arrow-elements-textSecondary">
        <div>
          <Checkbox
            checked={selectAllCheckedState}
            onCheckedChange={handleSelectAll}
            className="w-3 h-3 rounded border-arrow-elements-borderColor mr-2"
        ...
        {selectedItems.size > 0 && (
          <button
            className="ml-auto px-2 py-0.5 rounded bg-arrow-elements-button-secondary-background hover:bg-arrow-elements-button-secondary-backgroundHover text-arrow-elements-button-secondary-text text-xs flex items-center gap-1"
      ...
      <div className="flex-1 overflow-auto modern-scrollbar px-1 py-1">
        {filteredAndSortedItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-arrow-elements-textTertiary text-xs gap-2">
            ...
            {filteredAndSortedItems.map((item) => (
              <li
                ...
                className={classNames(
                  'text-arrow-elements-textTertiary flex items-center gap-2 px-2 py-1 rounded hover:bg-arrow-elements-background-depth-2 transition-colors group',
                  selectedItems.has(item.path) ? 'bg-arrow-elements-background-depth-2' : '',
                )}
              >
                <Checkbox
                  checked={selectedItems.has(item.path)}
                  onCheckedChange={() => handleSelectItem(item.path)}
                  className="w-3 h-3 rounded border-arrow-elements-borderColor"
                  aria-labelledby={`item-label-${item.path}`} // For accessibility
                />
                <span
                  className={classNames(
                    'shrink-0 text-arrow-elements-textTertiary text-xs',
                ...
                ></span>
                <button
                  className="flex items-center px-1 py-0.5 text-xs rounded bg-transparent hover:bg-arrow-elements-background-depth-3"
                  onClick={() => {
                    if (item.type === 'file') {
      ...
      {/* Footer */}
      <div className="px-2 py-1 border-t border-arrow-elements-borderColor bg-arrow-elements-background-depth-2 text-xs text-arrow-elements-textTertiary flex justify-between items-center">
        <div>

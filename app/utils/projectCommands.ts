...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<arrowAction type="shell">${commands.setupCommand}</arrowAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<arrowAction type="start">${commands.startCommand}</arrowAction>
`;
  }
...
<arrowArtifact id="project-setup" title="Project Setup">
${commandString}
</arrowArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeArrowArtifactTags(input: string) {
  // Regular expression to match boltArtifact tags and their content
  const regex = /(<arrowArtifact[^>]*>)([\s\S]*?)(<\/boltArtifact>)/g;
...
export function escapeArrowActionTags(input: string) {
  // Regular expression to match boltArtifact tags and their content
  const regex = /(<arrowAction[^>]*>)([\s\S]*?)(<\/boltAction>)/g;
...
export function escapeArrowTags(input: string) {
  return escapeArrowArtifactTags(escapeArrowActionTags(input));
}
